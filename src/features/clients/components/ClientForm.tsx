...
import type { Client, InsertTables } from '@/types/database'

type FormData = Omit<InsertTables<'clients'>, 'company_id' | 'id'>
...
export function ClientForm({ client, isLoading, onSubmit, onCancel }: ClientFormProps) {
  ...
  useEffect(() => {
    if (client) {
      setFormData({
        ...
        work_point_address: client.work_point_address || '',
        work_point_city: client.work_point_city || '',
        work_point_county: client.work_point_county || '',
        phone: client.phone || '',
        email: client.email || '',
        contact_person: client.contact_person || '',
        iban: client.iban || '',
        bank: client.bank || '',
        environment_auth: client.environment_auth || '',
        environment_auth_expiry: client.environment_auth_expiry || '',
        notes: client.notes || '',
        is_active: client.is_active,
      })
      setShowWorkPoint(!!(client.work_point_address || client.work_point_city))
    } else {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="flex items-center gap-2">
        ...
        <Label htmlFor="is_active" className="cursor-pointer">Client activ</Label>
      </div>
